...
func processBraille(braille string) ([][]bool, error) {
	lane := [][]bool{
		[]bool{},
		[]bool{},
		[]bool{},
...
func main() {
	conf := configFromArgs(os.Args)

	in, err := conf.getSource()
	if err != nil {
		log.Fatal(err)
	}
	defer in.Close()
	...
	for EOF := false; !EOF; {
		...
		rows, err := processBraille(strings.TrimSpace(content))
		if err != nil {
			log.Fatal(err)
		}
	...
	renderer, err := conf.getRenderer()
	if err != nil {
		log.Fatal(err)
	}
	svg := renderer.Render(braille)

	out, err := conf.getDestination()
	if err != nil {
		log.Fatal(err)
	}
	defer out.Close()
